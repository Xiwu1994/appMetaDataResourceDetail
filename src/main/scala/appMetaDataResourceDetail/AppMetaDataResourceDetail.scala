...
object AppMetaDataResourceDetail {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().config("yarn.timeline-service.enabled","false").appName("appMetaDataResourceDetail").getOrCreate()
    ...
    val app_res_db = sqlContext.sql(select_db_sql).rdd.map{ line =>
    ...
    val app_res_table = sqlContext.sql(select_table_sql).rdd.map{ line =>
    ...
    val app_res_column = column_rdd.map{ line =>
    ...
    // insert app
    app_res_db.union(app_res_table).union(app_res_column).map(_.split("\t")).map(e => {
